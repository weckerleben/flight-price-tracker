...
from app.config import SERVICE_ACCOUNT_FILE, SCOPES, SPREADSHEET_ID
...
def compare_and_update_sheet(departure_date, return_date, days, scraped_price, duration, URL):
    current_price_str = read_value_from_sheet('Kayak!E1')
    current_price_per_day_str = read_value_from_sheet('Kayak!F2')
    try:
        if scraped_price is not None:
            current_price = float(current_price_str)
            scraped_price_float = float(scraped_price)
            current_price_per_day = float(current_price_per_day_str.replace(',', '.'))
            price_per_day = round(scraped_price_float / days if days != 0 else 1, 2)

            if scraped_price_float < current_price:
                log.info(f"New lowest new price found: {scraped_price_float}. Updating sheet...")
                values = [
